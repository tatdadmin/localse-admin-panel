...
const BusinessCallForPaidSubscriptionRenewal = () => {
  ...
  const getData = async () => {
    try {
      setLoading(true);
      const res = await GET_DATA_FOR_CALL_FOR_SUBSCRIPTION_RENEWAL();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // return false
      const res = await UPDATE_DATA_FOR_CALL_FOR_SUBSCRIPTION_RENEWAL(payload);
      if (res?.status_code === 200) {
        ...
        {data && (
          <div>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              <p style={{ margin: "0.2rem 0", fontSize: "1.2rem" }}>
                <strong>Address:</strong> {data.business_address || "N/A"}
              </p>
              <p style={{ margin: "0.2rem 0", fontSize: "1.2rem" }}>
...
export default BusinessCallForPaidSubscriptionRenewal;
